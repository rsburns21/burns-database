...
async def codex_agent_control(params: CodexAgentParams) -> Dict[str, Any]:
    """
    Control the codex background agent.
    ...
    - add_task: Add a new task to the agent queue
    """
    import uuid
    ...
    try:
        if action == "start":
            # Start the codex agent as daemon
        ...
        return {
            "ok": False,
            "error": f"Agent control error: {e}"
        }

@mcp.tool(description="Get codex agent configuration and status.")
async def codex_agent_info() -> Dict[str, Any]:
    """Get information about the codex background agent configuration and status."""
    try:
...
async def codex_agent_create_config() -> Dict[str, Any]:
    """Create a sample configuration file for the codex background agent."""
    try:
        result = subprocess.run([
