...
# Version: 4.3.1 (CI fix: FastMCP __init__ kwargs compatibility)
...
# ---------------------------
# MCP Server (robust init across FastMCP versions)
# ---------------------------

def _create_mcp(name: str, version: str) -> FastMCP:
    """
    Handle FastMCP signature differences across versions:
    - Prefer (name, version=...)
    - Fallback to (name) if version kwarg unsupported
    """
    try:
        return FastMCP(name, version=version)  # new versions
    except TypeError:
        return FastMCP(name)  # older versions without version= kw
...
class CodexAgentParams(BaseModel):
    ...
    task_type: Optional[str] = Field(None, description="Task type: codex_analysis|document_processing|search_optimization|embedding_generation")
...
async def codex_agent_control(params: CodexAgentParams) -> Dict[str, Any]:
    ...
    try:
        if action == "start":
            ...
            task_data = {"task_id": task_id, "task_type": params.task_type, "payload": params.task_data, "created_at": _now_ms()}
